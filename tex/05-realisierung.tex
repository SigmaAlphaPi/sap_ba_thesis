...
Zunächst wurde das Trödeln mit einer Wahrscheinlichkeit von 10 $\%$ ausgeführt.
...
\\
Softwareseitig ist Zelle \texttt{0} die erste zu besetzende Zelle einer Lane. Die freie Zelle hinter dem Fahrzeug war Zelle \texttt{-1} und nicht die letzte Zelle am Lane-Ende.
...
\\
Anmerkung: Gleichzeitig reagierte \texttt{vehicle0} korrekt auf das Hindernis direkt vor sich und leitete einen Überholvorgang ein.
...
Die Festlegung der Anzahl der Fahrzeuge erfolgte aufgrunde realer Daten des BASt aus dem Jahr für eine Messstelle auf der A7 zwischen den Anschlussstellen Seesen und Echte (\cite{zaehlstelle}).
Zu diesem Zeitpunkt war dieses Autobahnstück noch zweispurig ausgebaut.

In Fahrtrichtung Hannover fuhren pro Tag durchschnittlich 25881 Pkw, in der Gegenrichtung 26340.
Schwerlastverkehr wurde hier vorerst außer Acht gelassen.
\\
Die o.a. Zahlen spiegeln eine Verkehrsstärke $q$ ($ \frac{\text{Fahrzeuge}}{delta t} $) wieder.
Nach \cite{verkehrsplanung} kann diese mit Hilfe der mittleren momentanen Geschwindigkeit $\overline{v}_{m}$ in eine Verkehrsdichte $D$ ($ \frac{\text{Fahrzeuge}}{delta s} $) umgerechnet werden - $ D = \frac{q}{\overline{v}_{m}} $.

Die Berechnung erfolgte mit 26111 Fahrzeugen, dem Durchschnittswert der beiden Fahrtrichtungen - Wert pro Stunde: 1088.
Als $\overline{v}_{m}$ wird für Pkw 120 km/h angenommen.
Somit ergibt sich auf diesem zweispurigen Autobahnstück eine Verkehrsdichte von 9 $\frac{\text{Fahrzeugen}}{\text{km}}$.
\\
Für das Szenario muss diese halbiert und auf die Streckenlänge hochgerechnet werden.
...
\subsection{Zellgröße/Zeitschrittlänge vs. Simulation realer Beschleuningung/Verzögerung}
...
\\
Mindestabstand: $ s_{min} = \lceil x \times Zellgr. \rceil $
\\
...
\begin{tabular}{| c  c  c |}
\hline 
(Rest-)Geschwindigkeit & $ s_{min} $ in m bei ts=0,025 min & $ s_{min} $ in m bei ts=0,05 min \\ \hline 
130 & 55 & 110 \\ 
100 & 43 & 85 \\ 
80 & 35 & 68 \\ 
70 & 30 & 60 \\ 
