...
\sa{own: [Grafik Speedcurve][Grafik Cellcurve] (got it)}
%\begin{figure}[hptb]
%  \centering 
...
Für die Trödelwahrscheinlichkeit $ p_{linger} $ wurde beobachtet, dass deren Höhe bei Werten $\leq$ \texttt{0,5} nur einen kleinen Einfluss auf das Verhalten bei max. möglicher Geschwindigkeit hatte.
In den Testdurchläufen konnte die Geschwindigkeit erreicht werden.
\\
...
Bei einem Wert von \texttt{0,7} ergab sich ein Durchlauf, bei dem die 20 km/h nicht erreicht wurden.
\sa{own: add 3 linger speed graphs 0,7}
\\
Eine merkliche Veränderung trat auch bei der Beschleunigung der Fahrzeuge auf.
Je höher der Wert für $ p_{linger} $, desto flacher wurde der Anstieg bei der Beschleunigung.
...
\sa{own: add 3 linger speed graphs 0,1 0,3 0,5}
...
Das Reaktion darauf kann über einen Agentenplan gesteuert werden.
...
\\
Durch die Entfernungen, die in einem größeren Zeitschritt zurückgelegt werden, wird durch eine Anhebung der Zeitschrittlänge auch der Mindestabstand entsprechend höher.
